...
### Документируй паттерны

Если видишь повторяющийся паттерн разработки — предложи задокументировать в [docs/principles.md](docs/principles.md).

Примеры паттернов:
- Архитектурные решения (read/write разделение, Env interface)
- Соглашения по коду (формат ошибок, именование)
