...
1. **SQL Queries**
   ...
   - Name variants:
      - GetEntityByID
      - ListAllEntities (without filters)
      - ListEntitiesByEmail (email filter)
      - InsertEnitity (insert
      - ActionEntity (update, delete)
...
2. **GraphQL Schema**
   ...
   - All mutations acept only one `input` argument.
   - All mutations return `union ${Mutation}OrErrorPayload = ${Mutation}Payload | ErrorPayload`.
