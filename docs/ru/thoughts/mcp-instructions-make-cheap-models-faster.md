...
Одна честная оговорка о том, что этот тест доказывает, а что нет. Вариант с заметкой кладёт её прямо в системный промпт модели. Он не запускает живое рукопожатие MCP-клиента, так что строго говоря я измерял «заметка в контексте», а не «реальный клиент её получил и показал модели».

Это всё равно честная замена — из-за того, куда заметка попадает в реальной схеме. Сервер trip2g возвращает содержимое заметки в стандартном поле `instructions` ответа `initialize`. Именно это поле совместимый с MCP клиент (Claude Code, Cursor и другие) при подключении подаёт в контекст модели. Вставить заметку в системный промпт — это смоделировать то, что делает нормальный клиент с `initialize.instructions`.

Оговорка к оговорке: реальная выгода зависит от того, действительно ли клиент показывает это поле. Некоторые клиенты обрезают инструкции или кладут их туда, где модель их меньше учитывает. Так что эти цифры — потолок для совместимого клиента, а не гарантия для любого.
