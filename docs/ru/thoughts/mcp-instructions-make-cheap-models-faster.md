...
Я попробовал подтвердить это от начала до конца и прошёл часть пути. Claude Code чисто подключается к базе как HTTP-сервер MCP, так что рукопожатие с реальным клиентом работает, но у тестового аккаунта кончился баланс, и посмотреть, как он реально отвечает на вопрос, я не смог. Codex запускается, но headless он слишком медленный и прожорливый по токенам, чтобы измерить его в отведённое время. Итого подтверждено: сервер отдаёт заметку в нужном поле и реальный клиент к нему подключается; пока не подтверждено: живой след того, как именованный клиент показывает это поле модели, а та идёт по дешёвому циклу. Считайте эти цифры потолком для клиента, который показывает `initialize.instructions`, а не гарантией для любого клиента.
