...
*Июль 2026 — вышел Grav 2.0 и сдвинул часть карты. [[ru/thoughts/flat-file-cms-revisited|Разбор с проверкой фактов]]: что устарело и где trip2g всё ещё один.*
...
Кастомный layout в trip2g — это полноценная HTML-страница с доверием админа. А значит, в неё можно положить SPA.
...
Получается вторая формула позиционирования: trip2g — это ещё и app-shell. «Принеси своё SPA — я дам хостинг, авторизацию, роутинг, ассеты; данные — из моих заметок или из твоего Postgres». Плюс: MCP-эндпоинт (те же заметки как инструменты для агента), платный доступ и федерация между хабами.

Grav 2.0 (вышел в июне 2026) теперь поставляется с первоклассным REST API, встроенным MCP-сервером и SPA-админкой на SvelteKit — и при этом остаётся чистой flat-file CMS без базы данных. Это ближайший сосед trip2g по духу, и в одном отношении более честный flat-file: каноничное хранилище у Grav — файлы, а не SQLite. Специфика trip2g — в другом: Obsidian-vault как поверхность авторинга с двусторонней синхронизацией, гибрид file-UX поверх SQLite и мультиканальная доставка — веб, Telegram, федерация — из одного бинарника.
...
**Операционная модель.** Сколько процессов надо крутить и живой ли сервер. Здесь trip2g стоит рядом с SSG по простоте (один бинарник), но по «живости» — на стороне WordPress. Именно эта клетка — «один процесс И живой сервер» — заполнена редко: Grav 2.0 (flat-file, без базы) — ближайший сосед в ней, рядом с SilverBullet.
...
*Диаграмма C: сколько частей надо собрать (один процесс ↔ собранный стек) и живой ли рантайм (статика ↔ сервер). trip2g — «один бинарник И живой сервер»; Grav 2.0 стоит в той же клетке — flat-file PHP, без базы, живой сервер.*
