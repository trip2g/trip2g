...
| F3: cross-encoder reranker (режим замены) | 1.000 | **0.888** | 0.871 | 0.879 |
| → отправили reranker отключённым | 1.000 | 0.922 | 0.942 | 0.860 |
| → вернули как blend (opt-in, w=0.5), перезамерили | 0.992 | **0.949** | 0.969 | **0.924** |
...
## F3: reranker — хуже как замена, лучше как примесь
...
В первой версии это было единственное изменение, которое ухудшило результаты.
...
Анализ по запросам объяснил механику. Cross-encoder поощряет поверхностное совпадение терминов. «Медленное брожение теста» — reranker поднял квашеную капусту выше закваски: у капусты больше словаря про брожение. «Каналы вместо общей памяти» — поднял мьютексы выше горутин. Первый этап (bi-encoder + RRF) правильно поставил их ниже ответа. Reranker, которому велели переупорядочить всё, эту работу отменил.

Это и есть диагноз, и он указывает на починку. Проблема была не в сигнале cross-encoder, а в том, что мы позволили ему затирать уже сильный первый этап. Поэтому мы поменяли две вещи и замерили заново. Примесь вместо замены: итоговая оценка `(1-w)·rrf + w·ce`, обе нормированы, так что cross-encoder подталкивает ранжирование, а не диктует его (`w=0.5`). И дисциплина окна: реранжируем только лучший пассаж заметки, всегда в пределах 512 токенов, никаких обрезанных целых заметок — ровно того входа, что дал обвал до 0.388.

В таком виде он выигрывает. nDCG 0.926 → 0.949, MRR 0.950 → 0.969, и сильнее всего там, где первый этап слабее всего: кросс-язык en→ru, 0.867 → 0.924 (+0.057). Зеркало результата в режиме замены.

Вывод уже, но полезнее, чем «рерранкеры не работают». Reranker, который переупорядочивает всё, вредит сильному первому этапу на маленьком корпусе с близкими темами. Та же модель, подмешанная к оценке первого этапа и накормленная только входами, влезающими в окно, вытаскивает то, что первый этап упускает — трудные кросс-языковые случаи — не ломая того, что и так было верно. Остаётся за флагом, по умолчанию выключен: нужен CPU-сайдкар cross-encoder и лишняя латентность, то есть это opt-in инфраструктура, а не бесплатный дефолт. Больше не отрицательный результат, а рабочая опция, которая себя окупает там, где сайдкар поднят.
