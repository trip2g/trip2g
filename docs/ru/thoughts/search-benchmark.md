...
Вывод: reranker не бесплатный. Когда первый этап уже сильный (~0.92 nDCG) и корпус плотно набит тематически близкими документами, «переупорядочить всё» вредит. Сначала мы отправили reranker отключённым (`vector_search.reranker.enabled=false`), а потом удалили код целиком: фича, которая по замерам делает хуже, не стоит лишних движущихся частей, и отрицательный результат зафиксирован в dev-доках. Более принципиальный подход — смешивать оценку reranker с рангом RRF вместо полной замены — оставлен как направление для дальнейшей работы.
