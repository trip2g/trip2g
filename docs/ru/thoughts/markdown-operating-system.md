...
Слова из операционных систем здесь не для красоты. За каждым стоит конкретный механизм. Вот соответствие, с пометкой, насколько оно реально. «Отгружено» — работает на `main`. Агентские строки легли последними: они жили в `feat/agent-runtime`, пока флот не влился.
...
| Оверлейная ФС | frontmatter-патчи (Jsonnet) переопределяют заметки, не трогая исходник | отгружено |
...
| Виртуальные хосты | маршрутизация по доменам через `route`/`routes` во фронтматтере | отгружено |
...
| Провайдеры входа | email magic-link, Google/GitHub OAuth, OIDC SSO | отгружено |
...
| Тикет-капабилити | HAT: подписанный токен с коротким TTL, повышение до админа через `ae=true` | отгружено |
| Дисплейный сервер | сайт: default- и Jet-шаблоны, mermaid, datachart | отгружено |
| Кеш страниц | кеш анонимных отрендеренных страниц, по версии | отгружено |
...
| Пульт управления | заметка-канбан (`layout: kanban`) под управлением агента | отгружено |
| Конфиг ядра | фиче-флаги, проверка на старте (паника при отсутствии зависимости) | отгружено |
| Исполнитель процессов | агентский цикл в заметке (`agentruntime`): LLM или детерминированный код | отгружено |
| Менеджер пакетов | роль-как-заметка: `fleet` регистрирует агента | отгружено |
| Лимиты ресурсов | жёсткие лимиты токенов и шагов на запуск | отгружено |
...
Это слой, который оправдывает слово «рантайм», и он уже на `main`: `fleet` и `agentruntime` влились в середине 2026-го и прогоняются сквозным тестом.
...
Модель нужна не каждому агенту. Роль может объявить `executor: code`: fenced-блоки её тела — это программа (python, bash или node), она выполняется в песочнице как конвейер stdout→stdin, а итоговый stdout разбирается как записи, которые проходят те же проверки доступа. LLM-агенту тоже доступен инструмент `exec` для запуска сниппетов, но только если оператор поднял `fleet` с флагом `--allowed-programs`; по умолчанию он выключен.
...
Обвязка под этим — вебхуки, ограниченные токены, задания доставки — отгружена. Отгружены и агентский цикл в заметке, и реконсайлер `fleet` поверх неё.
...
Канбан-лейаут, отдельный `kanban_template` и привязка доски к флоту отгружены; сквозной тест гоняет доску через живого агента.
...
Честная карта показывает свои края. trip2g — не редактор. Он оставляет редактирование на Obsidian и Telegram и берётся за дело только после того, как заметку сохранили. Так что если вы ждали операционную систему со своей оболочкой и окнами, парадная дверь — чужое приложение. Строка «менеджер пакетов» тоньше, чем звучит: это `fleet`, превращающий заметку в запись-вебхук, а не реестр с версиями и зависимостями. Код агенты выполняют только на условиях оператора: инструмент `exec` есть, но молчит, пока `fleet` не запустили с явным списком разрешённых программ, так что по умолчанию оболочки нет. Есть и зазор между словом и железом: прод-хабы крутятся на одной небольшой VM, которой уже тесно, поэтому «гоняет процессы-агенты» — обещание, которое текущая коробка держит только на малом масштабе.
