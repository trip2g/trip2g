...
0. **Возьмите готовое.** Весь пайплайн упакован в отдельный репозиторий [`krisp_knowledge`](https://github.com/trip2g/krisp_knowledge). Склонируйте его, скопируйте `.env.example` в `.env` и впишите `KRISP_TOKEN` и `OPENROUTER_API_KEY`. В комплекте синтетический `example/`-волт, чтобы увидеть форму результата ещё до прогона по своим звонкам.
