...
| `expand(pid, toc_path?, first?, last?)` | Обход оглавления заметки по уровням. Возвращает прямых детей узла оглавления, чтобы спускаться по структуре, не загружая заметку; раздел без подразделов возвращается целиком. `first` и `last` перечисляют только столько старейших и новейших детей — для заметки, у которой секций сотни. Подробнее: [[ru/user/expand]] |
...
expand(pid=42, last=30)                         → 30 новейших секций
expand(pid=42, first=5, last=30)                → оба края, середина не перечисляется
```
...
- `has_children` — есть ли у узла свои подразделы.

Что вернёт `expand`, зависит от раздела, который называет `toc_path`:

- Раздел с подразделами возвращает список детей, как описано выше.
- Раздел без подразделов возвращает своё содержимое — тот же текст, что дал бы `note_html(path, toc_path)`, плюс `section_html` в ответе, — так что второй вызов для чтения листа не нужен.
- Флага, который это отключает, нет, и клиенту, которому нужна только структура, он не нужен: родительский список уже помечает каждого ребёнка через `has_children`, и спуск останавливается на `has_children: false`.
