...
1. `search(query)` — получить результаты с `toc` и `toc_path` для каждого совпадения
2. Определить нужный раздел по `toc_path` ближайшего совпадения
3. `note_html(pid=N, toc_path=[...])` — загрузить только этот раздел
