...
Это принцип «всё есть заметка» в действии. Заметка-фид содержит frontmatter, который управляет тем, какие заметки попадают в ленту и сколько их. Движок передаёт их в `_layouts/rss.html` — Jet-layout, который лежит прямо в вашем хранилище Obsidian. Редактируйте его как любой другой файл и синхронизируйте. Шаблон поставляется в [[ru/user/onboarding-vault|стартовом архиве]], а исходный код опубликован на [github.com/trip2g/trip2g — rss.html](https://github.com/trip2g/trip2g/blob/main/onboarding-vault/_layouts/rss.html).
...
Старый автоматический формат `<адрес-заметки>.rss.xml` — по фиду на заметку, элементы = ссылки внутри неё — убран. Старые адреса подписчиков такого вида теперь возвращают 404, редиректа нет. Создайте заметку-фид как описано выше и обновите ссылки подписчиков на `/feed.xml`.
