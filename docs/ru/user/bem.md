...
### `$name` для уникальности имён блоков
...
`$name` решает проблему на уровне имён блоков. Перед разбором шаблона движок заменяет `$name` в именах блоков на значение, производное от пути к файлу:

- `components/button.html` → `$name` = `components_button`
- `ui/nav/header.html` → `$name` = `ui_nav_header`

Рекомендуемый паттерн сочетает BEM-именование классов с `$name` в именах блоков:
...
BEM делает уникальными CSS-классы на странице. `$name` делает уникальными имена блоков Jet во всех файлах компонентов. Используйте оба подхода в каждом файле компонента.

### Смотрите также

- [[ru/user/yield_blocks|yield_blocks: CSS и JS по страницам]] — как собираются блоки стилей и полная документация по `$name`
