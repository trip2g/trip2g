...
OIDC_DISPLAY_NAME="Войти через Telegram"   # надпись на кнопке
```
...
По умолчанию на кнопке написано **«Войти через SSO»** — читателю это не говорит ничего о том, куда он попадёт. Задайте провайдеру отображаемое имя, и на кнопке будет оно:

- провайдер через env: `OIDC_DISPLAY_NAME="Войти через Telegram"`
- провайдер из панели: поле `displayName` в настройках

Надпись показывается как есть, на всех языках: это имя, а не переводимая фраза. Оставьте пустым — вернётся текст по умолчанию.

## Вход по email

Если в инстансе все входят через SSO, форма входа по коду на почту не нужна, а на экране она только приглашает постучаться в дверь, которая всё равно не откроется.

Задайте `DISABLE_EMAIL_SIGNIN="true"` — и trip2g перестанет её предлагать: форма исчезает с экрана входа, а API отказывает во входе по коду. Остаются только кнопки SSO и OAuth.

То же самое умеет переключатель `email_signin_enabled` в настройках админки — но переменная окружения сильнее: пока она задана, включить вход по email из панели нельзя.

Прежде чем включать, убедитесь, что SSO работает: при выключенном входе по email и недоступном провайдере войти не сможет никто, включая вас.
