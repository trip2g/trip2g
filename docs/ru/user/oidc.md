...
| `email_verified` | Для каждого входа `/userinfo` должен вернуть `email_verified: true`; отсутствующий или ложный claim блокируется до поиска аккаунта trip2g по email. |
...
Настройте провайдер так, чтобы `email_verified` присутствовал в ответе UserInfo. Если claim отсутствует, trip2g блокирует вход, даже когда провайдер возвращает его только в ID token.
