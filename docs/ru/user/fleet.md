...
Секреты не ездят в окружении роли. Код-роль, которой нужен токен, несёт зашифрованное значение в собственном frontmatter и объявляет, какие поля открывать, — см. [[ru/user/codellm-secrets|Секреты для код-ролей]].
