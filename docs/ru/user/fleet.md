...
- **Код-роли**: программа на Python, Bash или Node в песочнице — для детерминированных шагов, которым модель не нужна.
...
| Ключ | Значение |
|------|----------|
| `fleet_id` | какой флит забирает роль. Он же задаёт тип запуска: флит за codellm исполняет тело как программу, флит с обычной моделью читает его как инструкцию. Роль без `fleet_id` не заберёт никто, и она не запустится |
...
| `env_passthrough`, `env_prefix` | только для код-ролей: какие переменные окружения получает код; сужают то, что разрешил оператор codellm |
| `unseal`, `unseal_env_key` | только для код-ролей: какие поля frontmatter содержат запечатанные секреты и каким ключом их открывать |
...
Не каждому шагу нужна модель. Пагинация, преобразование форматов, запрос к внешнему API — детерминированные задачи, и LLM здесь добавляет только стоимость и шум. Укажите роли флит, за которым стоит codellm, и тело запустится как программа:
...
# забрать новые записи и отдать изменения:
fleetkit.emit([fleetkit.note("transcripts/2026-07-02_standup.md", {"title": "Планёрка"}, "...")])
```
````

Ключа `executor` больше нет. Роль называет флит, флит решает, каким будет запуск, и та же заметка переезжает между ними правкой одной строки.

Программа — первый блок кода в теле (`python`, `bash` или `node`). Контекст доставки приходит JSON-файлом, путь к нему лежит в `$FLEET_INPUT`: его читает `fleetkit.bag()`, а `fleetkit.frontmatter()` отдаёт собственный frontmatter ролевой заметки — так настройки роли живут в той же заметке, что её объявляет. Программа печатает в stdout объект `{"changes": [...]}`, и каждое изменение проходит ту же проверку `write_patterns`, что и `write_note`. Запуск кода не обходит границы доступа.

Изоляция по умолчанию строгая, и она принадлежит codellm, а не флиту: дочерний процесс работает в песочнице на уровне ОС (Linux-неймспейсы плюс ограничение файловой системы через Landlock, сеть закрыта, пока оператор её не разрешит), а на неподдерживаемых системах отказывает запуск, а не деградирует молча. Пока оператор codellm не разрешит конкретные интерпретаторы, код не запускается вовсе.

Код-роль, которой нужен токен, либо читает его из окружения запуска, которое наполняет оператор codellm, либо несёт зашифрованное значение в собственном frontmatter. Оба пути разобраны в статье [[ru/user/codellm-secrets|Секреты для код-ролей]].
...
./fleet \
  ...
  --trip2g-admin-personal-token "<персональный токен хаба>" \
...
У каждого флага есть переменная окружения `TRIP2G_FLEET_<ФЛАГ>`. `--llm-base-url` принимает любой OpenAI-совместимый эндпоинт, включая локальную модель — или инстанс codellm: именно это и превращает флит в тот, что исполняет тела как код.

Песочница и список разрешённых интерпретаторов — настройки codellm, а не флита. Сам флит код не запускает.

`--trip2g-admin-personal-token` — единственный доступ флита к хабу: [[ru/user/mcp|персональный токен]] администратора. Вручную его выпускать не нужно: задайте на хабе `OWNER_PERSONAL_TOKEN_VALUE` со значением вида `t2g_…` — хаб сам заведёт токен владельцу при старте, — и передайте флиту то же значение. Подписывающий секрет хаба флит не держит, поэтому доступ — одна отзываемая строка: отзовите её в админке, и флит остановится за полминуты, без перезапуска хаба. Перезапуск отозванную строку не вернёт — чтобы вернуть доступ, задайте новое значение.
