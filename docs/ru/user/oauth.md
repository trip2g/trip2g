...
## Как выключить вход по email

Поле для email — дверь, которую trip2g держит открытой по умолчанию, и при отключении провайдера все возвращаются к ней. Но если аккаунты в инстансе заводятся только через провайдера, эта дверь не открывается ни для кого: читатель запрашивает код для несуществующего аккаунта и считает это вашей поломкой.

Задайте `DISABLE_EMAIL_SIGNIN="true"` в переменных окружения — и trip2g перестанет её предлагать. С экрана входа исчезают поле, кнопка и капча, а API отказывает во входе по коду напрямую, так что запросить его в обход экрана тоже нельзя. Остаются только кнопки провайдеров.

В настройках админки есть переключатель `email_signin_enabled` — то же самое, но на лету и без перезапуска. Переменная окружения сильнее: пока она задана, включить вход по email из панели нельзя.

Прежде чем включать, убедитесь, что хотя бы один провайдер работает. При выключенном входе по email и неработающих провайдерах войти не сможет никто, включая вас.

На кнопках Google и GitHub всегда написано «Войти через Google» и «Войти через GitHub» — читателю нужно имя провайдера. Переименования требует как раз OIDC: «SSO» называет протокол, а не то, куда попадёт человек. Про `OIDC_DISPLAY_NAME` — в [[ru/user/oidc]].
...
- [[ru/user/oidc]] — OIDC / Корпоративный SSO (Authentik и другие OpenID Connect-провайдеры)
