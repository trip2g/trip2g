...
**Шаг 1 — нечёткое совпадение.** Векторный поиск возвращает результаты. В каждом совпадении есть breadcrumb-путь к разделу, где нашлось совпадение: `matches[].toc_path` — самый вложенный заголовок, содержащий фрагмент. Результаты «тонкие»: полного оглавления заметки в них нет. Остальную структуру раскрывайте по уровням через [[expand]].

**Шаг 2 — определение раздела.** Поле `toc_path` указывает раздел в структуре заметки. Для примера выше это:
...
4. Нужен соседний раздел?
   ...
   → навигация без второго запроса search; раздел без подразделов
     приходит как содержимое, как из note_html
```
...
- Путь в breadcrumb должен совпадать с текущими заголовками заметки. Если заголовок переименован после индексации, путь не разрешится до следующего переиндексирования. В этом случае `note_html` вернёт ошибку со списком разделов верхнего уровня — дальше идите через `expand`, а не загружайте заметку целиком.
