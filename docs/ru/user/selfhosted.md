...
Две фоновые задачи по умолчанию запускаются **каждую минуту**:

- `send_scheduled_telegram_publishposts` — отправляет запланированные посты в Telegram.
- `execute_cron_webhooks` — запускает ваши cron-вебхуки по расписанию.

Каждая делает небольшую запись в базу на каждом тике. На нагруженном инстансе частоту можно снизить cron-выражением (6 полей, **с ведущим полем секунд**):

```dotenv
# Запускать обе раз в час вместо каждой минуты
CRON_TELEGRAM_PUBLISH_SCHEDULE=0 0 * * * *
CRON_EXECUTE_WEBHOOKS_SCHEDULE=0 0 * * * *
```

Бóльший интервал означает, что запланированные посты и cron-вебхуки срабатывают менее точно (с задержкой до часа) — в обмен на меньшую нагрузку на базу.

> Инстансы в облаке (managed) по умолчанию запускают их **раз в час**, чтобы не нагружать общую базу. Self-hosted-инстансы сохраняют дефолт «каждую минуту», пока вы его не измените.
