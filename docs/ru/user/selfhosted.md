...
- `SMTP_HOST` / `SMTP_USER` / `SMTP_PASS` — параметры SMTP. В примере используется шлюз Resend (`smtp.resend.com`, пользователь `resend`, пароль = API key). Без настроенного SMTP сервер запустится, но письма входа отправляться не будут — см. ниже про обходной путь для первого входа.
...
**Если SMTP не настроен**: письмо со входом отправить некому, поэтому код получить нельзя — добавьте `LOG_SIGN_IN_CODES=true` в `/etc/trip2g.env`, перезапустите (`systemctl restart trip2g`), запросите вход и заберите код из `journalctl -u trip2g`. Флаг печатает коды в открытом виде, поэтому отключите его (или настройте SMTP) сразу после первого входа.
