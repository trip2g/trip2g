...
| `graphql_request(query, variables?)` | Выполняет любой запрос или мутацию с правами администратора. Полный доступ на запись — запросы и мутации проходят без ограничений. Подписки не поддерживаются на этом транспорте. |
...
> **Федеративный путь (peer):** Когда другой инстанс trip2g вызывает `graphql_request` через federation secret, запрос идёт по федеративному пути — только для чтения и ограничен одобренным списком корневых полей. Только admin-путь (API-ключ с `enable_mcp_admin_tools`) имеет доступ на запись.
