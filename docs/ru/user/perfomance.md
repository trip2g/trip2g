...
| DigitalOcean basic | 1 / 512 МБ | **$4/мес** | упёрлось в сеть* | **0 дропов** |

\*$4-дроплет стоял в Нью-Йорке, а генератор — в Германии (RTT 86 мс), так что его rps упёрся в трансатлантический канал (p50 ≈ те же 88 мс RTT), а не в CPU. Свой потолок у него выше; чтобы померить — нужен генератор в том же регионе. Цены — list-прайс провайдеров.
