...
Мы залили эту самую документацию — 35 реальных страниц (набор `/en/user`: кастомный HTML внутри default-темплейта) — на Hetzner **cpx32** (4 vCPU AMD EPYC-Genoa, 8 ГБ) и нагрузили их [vegeta](https://github.com/tsenart/vegeta), round-robin по всем страницам (~52 КБ каждая), в production-режиме:

| Нагрузка | Успех | p50 | p95 | p99 |
|------:|:-----:|----:|----:|----:|
| 1 000 зап/с | 100 % | 1.3 ms | 2.5 ms | 11 ms |
| 2 000 зап/с | 100 % | 6.9 ms | 38 ms | 62 ms |
| 3 000 зап/с | 100 % | 212 ms | 513 ms | 740 ms |

Одна 4-ядерная нода отдаёт **~2 000 реальных страниц в секунду** со 100 % успеха и p99 ~60 ms; колено — около 2 500–3 000 зап/с, дальше задержка уходит в сотни мс. Это потолок одной ноды на таком железе и точка, где окупается вторая нода (или read-реплика — см. [[zerodowntime]] / [[litestream]]). Замерено 2026-06-22.

Основную стоимость запроса на этом потолке дают два пункта: **gzip-сжатие** каждого ответа и несколько **DB-запросов на запрос** в render-пути (проверка доступа, встроенные Telegram-ссылки, HTML-инъекции, трекинг просмотров). Оба кэшируемы — страницы статичны между записями — так что есть явный запас, чтобы будущим проходом оптимизации поднять потолок в разы.
