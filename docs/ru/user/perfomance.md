...
chart_ramp: "[[pagecache_ramp_ru.datachart.csv]]"
---
...
\*Одно ядро держит **~500 rps** страницы `/` (~43 КБ) на 100 % с p99 < 50 мс; колено ~600–700 rps, за ~900 латентность разваливается. Замерено с генератором в том же US-East (~23 мс) — первый прогон с генератором в Германии дал обманчивые «1000 rps», но тот трансатлантический канал 86 мс просто throttлил генератор, и дроплет не получал реальную нагрузку. Цены — list-прайс провайдеров.
...
```datachart
{
  ...
  "config": {
    "title": { "text": "Задержка vs. темп запросов, cx23 (меньше — лучше)" },
    "tooltip": { "trigger": "axis" },
    "legend": {},
    "xAxis": { "type": "category", "name": "зап/с" },
    "yAxis": { "type": "log", "name": "мс" },
    "series": [
