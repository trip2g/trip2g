---
free: true
title: Ограничения публикации в Telegram
---

После публикации пост в канале остаётся связан с заметкой: правите заметку в Obsidian, синхронизируете — пост обновляется. Но обновляется не всё. Текст, форматирование, ссылки и списки меняются свободно; медиа, тип поста и размер альбома Telegram замораживает после публикации — их меняют только через «Сбросить» в админке. Ниже — что обновляется само, что требует сброса и какие действуют лимиты по длине.
...
## Добавление тега после публикации

**Новый тег не догонит опубликованную заметку.**
Добавили в `telegram_publish_tags` ещё один тег после того, как пост уже вышел — в новый канал он не уедет. Состояние «опубликовано» хранится на заметке целиком, а не отдельно по каналам.

**Что делать:**
1. Откройте админку → «Telegram посты» → пост → «Сбросить»
2. Синхронизируйте заметку заново — она уйдёт во все подходящие каналы

Сброс удаляет пост из всех каналов, где он уже вышел, и публикует заново с новыми ID сообщений: ссылки на старый пост перестанут работать. Если это не подходит, опубликуйте во второй канал из отдельной заметки.

## Полезные советы

**1. Тестовый канал**
Создайте отдельный канал для экспериментов. Настройте Instant Tags — посты будут публиковаться моментально после синхронизации. Подробнее: [[ru/user/telegram-instant|предпросмотр в тестовом канале]].
...
Как использовать жирный, курсив, ссылки, код, спойлеры — читайте на странице [[telegram_syntax|Разметка для Telegram]].
