...
- **Cloudflare Turnstile** (`turnstile: true`) — принимается в spec, но не проверяется. Защита от спама сейчас опирается на `can_submit: admin` или существующую систему банов.
