...
`expand` — инструмент MCP для навигации по оглавлению заметки уровень за уровнем. Агент спускается по дереву секций и читает только нужный лист — не загружая ни всю заметку, ни плоское оглавление целиком. Результат: в разы меньше токенов и ответ остаётся в верхней части контекста, где модель работает точнее.

`expand` — инструмент MCP, который раскрывает оглавление заметки **по одному уровню за раз** (progressive disclosure). Агент спускается по дереву секций сверху вниз и читает только нужный лист — не загружая ни всю заметку, ни её полное плоское оглавление.
