...
| `BackLinks(note)` | Обратные ссылки (кто ссылается на эту заметку) — только те, что доступны текущему читателю |
...
`BackLinks` и `OutLinks` отдают только заметки, доступные текущему читателю. Недоступной заметки в списке нет вовсе — ни заголовка, ни ссылки, — если только каждый её подграф не помечен флагом **Teaser** («Витрина»). Проверять права в шаблоне не нужно и не получится: у шаблона нет личности читателя, фильтр стоит до него. См. [[ru/user/subgraphs#podgrafyi_vitrinyi|Подграфы-витрины]].
