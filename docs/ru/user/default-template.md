...
Дефолтный шаблон — встроенный макет страницы, который trip2g использует по умолчанию. Он даёт всё сразу: шапка, подвал, боковые панели, оглавление, обратные ссылки и сетка карточек для связанных заметок.
...
Добавьте `wide: true` во frontmatter заметки, чтобы отрисовать её на всю ширину: страница убирает обе боковые панели и узкую читаемую колонку, и широкий контент — kanban-доски, большие Mermaid-диаграммы, широкие таблицы — занимает всю страницу.
...
`Backlinks`, `inlinks` и `outlinks` перечисляют только те заметки, которые текущий читатель может открыть. Недоступную заметку в список не кладут вовсе — ни заголовка, ни ссылки, — если только каждый её подграф не помечен флагом **Teaser** («Витрина»): тогда она стоит в списке как обычная. См. [[ru/user/subgraphs#podgrafyi_vitrinyi|Подграфы-витрины]]. `similar` фильтровался так всегда.
...
Если в заметке заданы **и** поле `title` во frontmatter, **и** ведущий `# Заголовок`, то `title` из frontmatter всё равно побеждает как значение заголовка — именно оно попадает в `<title>`, OpenGraph-теги и карточки. H1 в тексте при этом распознаётся, поэтому шаблон не добавляет собственный элемент заголовка. Наверху страницы остаётся ровно один заголовок — `# Заголовок` из текста — и двойного заголовка не возникает, даже если его текст отличается от `title` во frontmatter.

### Макет сетки карточек

Сетка отображает связанные заметки в виде карточек с трёхуровневой иерархией: featured (большая, на всю ширину), сетка (средние карточки), список (минимальные строки). Уровни настраиваются тремя плоскими ключами frontmatter на странице, где размещена сетка:

| Ключ | По умолчанию | Значение |
|------|--------------|----------|
| `magazine_featured` | `1` | Сколько первых заметок становятся крупными featured-карточками |
| `magazine_grid` | `4` | Сколько заметок после этого попадают в среднюю сетку |
| `magazine_grid_columns` | `2` | Число колонок в сетке |

Модель простая: заметки берутся по порядку. Сначала `magazine_featured` штук становятся featured-карточками, затем следующие `magazine_grid` попадают в сетку, а всё, что осталось, идёт в список. Специальных значений вроде «без ограничения» не нужно: установите счётчик в `0`, чтобы пропустить уровень.

```yaml
magazine_featured: 0
magazine_grid: 0
```
Обычный список: все заметки одного маленького размера.

```yaml
magazine_featured: 0
magazine_grid: 8
```
Без featured-карточки, 8 заметок в сетке, остальное уходит в список.

Если счётчик больше числа доступных заметок, он просто ограничивается сверху, без ошибки.
...
**`magazine_featured`** — сколько первых заметок отображаются крупными featured-карточками

```yaml
magazine_featured: 2
```

По умолчанию: `1`. Установите `0`, чтобы не было featured-карточки.

**`magazine_grid`** — сколько заметок после featured-уровня попадают в среднюю сетку

```yaml
magazine_grid: 6
```

По умолчанию: `4`. Установите `0`, чтобы пропустить уровень сетки.

**`magazine_grid_columns`** — число колонок в уровне сетки
