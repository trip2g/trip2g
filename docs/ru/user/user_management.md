...
1. `createUser(email)` → получить `userId`
2. `createUserSubgraphAccess(userId, subgraphIds)` → открыть доступ к контенту
3. При необходимости `createAdmin(userId)` → полные права администратора
...
### Шаг 2. Открыть доступ к подграфу
...
### Шаг 3. Назначить администратора (необязательно)
