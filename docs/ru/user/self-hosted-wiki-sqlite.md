...
- **SQLite — однопишущий.** Для личной или небольшой командной вики это не проблема: чтения отдают заранее отрендеренный HTML за единицы миллисекунд независимо от размера хранилища (см. [[ru/user/perfomance|производительность]]). Под большую нагрузку у trip2g есть свой ответ на масштабирование — [[ru/user/read-replica|read-replica]] (LiteFS плюс проброс записи на лидера): реплика отдаёт чтения локально, а записи форвардит на лидера, вдобавок к непрерывной репликации Litestream. Клиент-серверная база всё же масштабируется дальше при экстремальной конкурентной записи, так что учтите это, если она ожидается.
