...
Код-роли, которая ходит во внешний API, нужен токен. Открытым текстом в заметке его не положишь: волт синхронизируется через Obsidian, лежит в git и попадает в бэкапы. В окружении сервера — работает, но тогда добавить секрет значит поправить compose и перезапустить, и каждая код-роль на этом инстансе видит все значения.

Третий путь — тот же, что у Rails с `credentials.yml.enc`: зашифрованное значение коммитится, ключ остаётся снаружи. Ролевая заметка несёт шифротекст, ключ держит codellm, и значение расшифровывается для одной этой роли в момент запуска.
...
- [Запасной путь через окружение](#запасной-путь-через-окружение)
...
## Запасной путь через окружение

Запечатывать необязательно. Роль, которая не объявила ни одного поля в `unseal`, по-прежнему читает то, что оператор codellm разрешил в окружение запуска через `CODELLM_EXPOSE_ENV`:

```python
token = os.environ["KRISP_TOKEN"]
```

Этот путь работает и годится для значения, которое оператор ставит один раз и которое можно видеть всем код-ролям. Запечатывать стоит, когда секрет принадлежит одной роли или когда добавить секрет нужно без редеплоя.
