...
**До любого фреймворка проверьте базовый уровень.** Если ваш агент — Claude Code, у него уже есть встроенная память: `CLAUDE.md` плюс авто-память (собственные заметки Claude под `~/.claude/`), включённая по умолчанию. Она бесплатна и не требует настройки, но локальна для машины, best-effort и не разделяется с командой, поэтому в матрице ниже как фреймворк её нет. Работаете в одиночку на одной машине — встроенной памяти (или локального плагина вроде MemPalace, MIT, на устройстве) может хватить; инструменты отсюда оправдывают себя, когда память должна быть общей, долговечной или запрашиваемой на масштабе. Разбор «сначала встроенная» — в [[ru/user/claude-code-persistent-memory|постоянной памяти для Claude Code]].
