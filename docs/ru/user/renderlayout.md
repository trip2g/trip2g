...
- `warnings` — объект с тремя ключами: `layout`, `note`, `files`. Каждый содержит массив ошибок компиляции и рантайма Jet для своей области. Все массивы пусты = чистый рендер.
...
|-----|-------------|------------|
| `?preview_id=xxx` | нет | HTML для этого ID |
...
Проверяем, что уже задеплоенный лейаут рендерит конкретную заметку без ошибок.
...
Агент, работающий над лейаутом совместно с человеком, может показывать промежуточный результат без деплоя и без загрузки в vault:
