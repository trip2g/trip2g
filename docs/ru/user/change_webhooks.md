...
**Вебхук срабатывает повторно** — агент записывает заметку через API, это может триггерить вебхук снова. Если агент записывает то же содержимое, что уже хранится, запись не создаёт новую версию и не поднимает событие изменения — цепочка обрывается сама. Если агент действительно изменил заметку, защитой служит `max_depth: 1` (значение по умолчанию): вебхук срабатывает только на прямые изменения и не реагирует на правки от самого агента.
