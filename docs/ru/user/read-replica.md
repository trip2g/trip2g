...
Read-реплика разделяет нагрузку: второй экземпляр trip2g обслуживает публичное чтение локально, а первичный сервер принимает все записи. Это снижает задержку чтения, снимает нагрузку с первичного и позволяет перезапускать его без прерывания читателей. Смотрите также [[zerodowntime]] и [[litestream]].
...
- Два сервера в одной приватной сети (в примерах — приватная сеть Hetzner: первичный `10.20.0.2`, реплика `10.20.0.3`)
- Бинарник `litefs` на обоих серверах (`/usr/local/bin/litefs`)
- Бинарник `trip2g` на обоих серверах

Важно: Litestream (непрерывный S3-бэкап) и LiteFS (живая репликация) — разные инструменты, работающие с одним WAL. Если нужны оба — запускайте Litestream на отдельном первичном, не на LiteFS-монтировании. Подробнее в [[litestream]] и [[backup]].
...
`/litefs` — FUSE-монтирование, которое видит приложение. `/var/lib/litefs` — внутреннее состояние LiteFS (LTX-страницы). В `/litefs` могут лежать только SQLite-файлы баз данных.
...
Собирайте бинарник локально и передавайте на серверы — не собирайте на сервере:
...
`TRIP2G_LEADER_ADDR` — единственный переключатель. Если переменная задана, включается режим только для чтения.
...
**Проксирование записей работает — POST на реплику выполняется на лидере и возвращается обратно:**
...
**Аутентификация проверяется — прямой POST на внутренний порт лидера без заголовка даёт 401:**
...
Все 60 запросов прошли успешно. Реплика не прерывала обслуживание. Подробнее про процедуру деплоя — в [[zerodowntime]].
...
Смотрите также [[backup]], [[litestream]], [[zerodowntime]].
