...
- своё имя куки (`TRIP2G_SESSION_COOKIE_NAME`), чтобы избежать конфликтов сессий
...
Четвёртый пункт стоит выделить отдельно: внутренний порт лидера — не публичная точка записи. Любой клиент, который минует реплику и пишет напрямую на `:8082` без корректного HMAC, получает 401. Это требование соблюдается независимо от того, какая реплика пересылала запросы.
