...
Значение должно быть вики-ссылкой на ту же страницу на другом языке. На английской странице укажите ссылку обратно на русскую:
...
Когда задан `lang_redirect`, в шапке страницы автоматически появляется переключатель языка. Он показывает текущий язык с иконкой флага и полным названием на родном языке. По клику открывается выпадающий список с доступными альтернативами.

Флаги встроены для десяти языков:

| Значение `lang` | Флаг | Отображается как |
|---|---|---|
| `en` | 🇺🇸 | English |
| `ru` | 🇷🇺 | Русский |
| `de` | 🇩🇪 | Deutsch |
...
Значение `lang` нечувствительно к регистру. Поддерживаются распространённые псевдонимы — `"English"`, `"RU"`, `"German"` нормализуются в канонический код.

Для остальных языков флаг не показывается, если не добавить его через пользовательский CSS:
...
Замените `uk` на код вашего языка (строчными буквами) и укажите путь к изображению флага в `background-image`.
...
Порядок карточек задаёт свойство `home_position` на каждой заметке:
