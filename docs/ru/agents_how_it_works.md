...
### Тип роли executor: code

Второй вид роли (`executor: code`) запускает Python, Bash или Node для шагов, которым языковая модель не нужна: пагинация, маппинг полей, преобразование форматов. Тот же формат заметки-роли, тот же механизм триггеров — без расходов и задержки LLM. Программа работает в песочнице на уровне ОС с вычищенным окружением, а её записи проходят ту же проверку `write_patterns`, что и `write_note`. Полный справочник и запуск демона — в статье [[ru/user/fleet|Флит: агентский сайдкар]].
