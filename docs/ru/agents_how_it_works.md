...
Второй вид роли запускает Python, Bash или Node для шагов, которым языковая модель не нужна: пагинация, маппинг полей, преобразование форматов. Роль выбирает его, указав флит за codellm — сервисом исполнения кода; формат заметки и механизм триггеров те же, без расходов и задержки LLM. Программа работает в песочнице на уровне ОС, а её записи проходят ту же проверку `write_patterns`, что и `write_note`. Полный справочник и запуск демона — в статье [[ru/user/fleet|Флит: агентский сайдкар]].
