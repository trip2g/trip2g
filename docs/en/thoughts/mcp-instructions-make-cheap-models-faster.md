...
I confirmed it end to end with Claude Code, on its flat subscription rather than a per-token API key. The trick is to unset `ANTHROPIC_API_KEY` before launching the CLI: with the key set it bills the metered console path and stops on an out-of-credit error, which is the wrong way to run evals; with it unset the CLI falls back to the logged-in subscription and just runs. I added the base as an HTTP MCP server, then asked it the federation question with tool calls visible. Its trace was `search("federation private peer HMAC secret setup")` followed by `note_html(pid=658, toc_path=["Adding a private peer (two-step exchange)"])`. That is the taught loop exactly: search for a pointer, then read one section by its `toc_path`, no whole-note dump. It answered correctly and cited the note.

So a real client on the billing path people actually deploy does drive the drill-down loop. One honest limit: a single trace cannot fully separate "the client surfaced the note and the model obeyed it" from "the model would have drilled down anyway", since the without-note arm above shows models sometimes do that unprompted. What the live run settles is that the loop works in a real client on subscription billing; the A/B measures how much the note shifts that behavior. (Codex I left alone: headless it is slow and token-heavy, and a bounded check was not the place to wire it up.)
