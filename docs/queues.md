...
**Jobs:**
- `backjobs:extract_notion_pages` - Extract and process Notion pages
- `backjobs:send_sign_in_code` - Send sign-in code emails to users
...
- `backjobs:update_all_chat_telegram_publish_posts` - Update all telegram posts for a specific chat
...
Jobs in the Telegram API Queue implement automatic rate limit handling:
...
Job execution is tracked in the database with:
