...
When translating a component to another language (e.g., Russian):

1. **Find the English locale file**: Look for `-view.tree/*.view.tree.locale=en.json` file in the component directory
2. **Read the English translations**: The `.locale=en.json` files are automatically generated by the builder
3. **Create the target locale file**: Create or update `*.view.tree.locale=ru.json` (or other locale) file at the component root directory
4. **Translate the strings**: Copy the structure from the English file and translate each value

**Example**:
```
Source: assets/ui/admin/backgroundqueue/show/-view.tree/show.view.tree.locale=en.json
Target: assets/ui/admin/backgroundqueue/show/show.view.tree.locale=ru.json
```
