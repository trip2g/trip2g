...
В верхней панели страницы пользователя (`/admin#!nav=users/users_nav=users/id=<id>`),
рядом с **Make Admin** и **Edit**, есть кнопка **Sign-in link**: она вызывает
`createHatLink` для email этого пользователя, а в карточке появляются готовая
ссылка, срок её жизни и кнопка **Copy**. `adminEnter` не
передаётся — ссылка входит под самим пользователем; права админа выдаются
отдельной кнопкой **Make Admin**.

Ссылку нужно копировать кнопкой **Copy**, а не пересказывать её текстом: JWT
длинный, и в мессенджерах (или в ответе LLM-агента) он легко теряет середину —
в рабочем токене ровно две точки.

Реализация: `assets/ui/admin/user/hatlink/`.
