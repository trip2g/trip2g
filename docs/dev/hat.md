...
На странице пользователя (`/admin#!nav=users/users_nav=users/id=<id>`) есть кнопка
**Sign-in link**: она вызывает `createHatLink` для email этого пользователя и
показывает готовую ссылку, срок её жизни и кнопку **Copy**. `adminEnter` не
