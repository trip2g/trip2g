...
| Delivery cleanup: 30 дней | `webhook_delivery_logs` — 1 день (тела ответов агентов бывают в сотни КБ), `deliveries` — 30 дней, `job_statuses` — 30 дней |
...
## Цепочки доставок (trace)

Один прогон агента порождает следующий: cron пишет заметку → срабатывает change-вебхук → его агент пишет ещё одну → срабатывает следующий. Всё это может обслуживаться разными инстансами fleet, но проходит через trip2g, поэтому цепочка целиком видна здесь.

Каждая строка доставки (обе таблицы) несёт четыре колонки:

| Колонка | Смысл |
|---------|-------|
| `trace` | Идентификатор цепочки — сериализованный корень, `cron:4242`. Корень несёт собственный id, потомки наследуют его без изменений |
| `parent_kind` + `parent_id` | Доставка, чья запись вызвала эту. У корня — `null`. Пара, а не один id: автоинкременты у двух таблиц независимы |
| `depth_reached` | Глубина этой доставки в цепочке. Не путать с `max_depth` вебхука — то настроенный лимит |

Родитель не вычисляется по данным: он уже едет в запросе. Скоупленный `api_token` доставки несёт `DeliveryKind`/`DeliveryID`, они штампуются в `appreq`, и в момент создания дочерней доставки `handlenotewebhooks` берёт их оттуда. Читается только `trace` родителя — одним запросом по первичному ключу.

Корнями становятся: cron-запуск (по расписанию или вручную), ручной триггер change-вебхука и любая запись, сделанная человеком, api-ключом или синхронизацией вольта — то есть всё, за чем не стоит доставка.

`trace` уезжает и в payload доставки, чтобы агент мог пометить им свои собственные логи.

Смотреть: админка → System → «Цепочки доставок». Список — `admin { deliveryTraces }`, отдельная цепочка — `admin { deliveryTrace(trace:) }`, оба собирают строки из обеих таблиц доставок.

Отдельная таблица трейсов не заводилась: агрегаты (токены, длительность) считаются на чтении, а чистка достаётся бесплатно от тех же 30-дневных DELETE.
