...
Встроенный mock-сервер для тестирования вебхуков. Доступен только при `DEV_MODE=true`.

### `POST /debug/test_webhook`

Mock-эндпоинт, принимающий delivery вебхука, сохраняющий в памяти и отвечающий настраиваемым ответом.

```
POST /debug/test_webhook?status=200&delay=0s&body={...}
-> сохраняет request headers + body в in-memory store
-> отвечает указанным статусом после задержки
```

Query params:
- `status` (int, дефолт 200) — HTTP status code
- `delay` (duration, дефолт 0s) — задержка перед ответом (для тестирования таймаутов)
- `body` (string, опционально) — JSON response body. Если не задан и mode=echo -> возвращает полученный payload обратно

Режимы:
- **echo** (дефолт, когда `body` не задан): возвращает полученный payload обратно (проверка корректности payload)
- **static** (когда `body` задан): возвращает указанное тело (тестирование flow ответа агента)

Примеры:
- `?status=500` — тестирование retry при HTTP-ошибке
- `?delay=120s` — тестирование таймаута
- `?status=200&body={"changes":[{"path":"test.md","content":"# Test"}]}` — тестирование flow ответа агента

### `GET /debug/test_webhook_calls`

Возвращает все сохранённые вызовы.

Query params:
- `last=1` — вернуть только последний вызов (удобно для assertions)

```json
[
  {
    "timestamp": 1738000000,
    "headers": {"X-Webhook-ID": "42", "X-Webhook-Signature": "sha256=..."},
    "body": {"id": 42, "changes": [...]}
  }
]
```
...
### Реализация
- In-memory `[]WebhookCall` с mutex
- Роуты регистрируются только при `DEV_MODE=true`
- Без БД, без goqite
...
E2E тесты используют Playwright `request` API (без браузера) + debug endpoints. HMAC верификация через Node.js `crypto.createHmac`.
...
- Проверить: changes содержит только `blog/post.md`
...
- Mock: возвращает changes с тем же путём (попытка цикла)
