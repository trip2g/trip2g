...
# Get imported account and bot IDs
ACCOUNT_IDS=$(sqlite3 "$DB_PATH" "select id from telegram_accounts")
BOT_ID=$(sqlite3 "$DB_PATH" "select id from tg_bots limit 1")

# Create tg_bot_chat for the bot (test channel)
if [ -n "$BOT_ID" ]; then
    sqlite3 "$DB_PATH" "insert or ignore into tg_bot_chats (telegram_id, chat_type, chat_title, can_invite, bot_id) values (-1001234567890, 'channel', 'Test Channel', 0, $BOT_ID)"
    CHAT_ID=$(sqlite3 "$DB_PATH" "select id from tg_bot_chats where telegram_id = -1001234567890")
...
for account_id in $ACCOUNT_IDS; do
    sqlite3 "$DB_PATH" "insert or ignore into telegram_publish_account_chats (account_id, telegram_chat_id, tag_id, created_by) values ($account_id, -1001234567890, $TAG_ID, 1)"
    sqlite3 "$DB_PATH" "insert or ignore into telegram_publish_account_instant_chats (account_id, telegram_chat_id, tag_id, created_by) values ($account_id, -1001234567890, $TAG_ID, 1)"
