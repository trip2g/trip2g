...
// TestRun_EmitsFleetEnvMessage asserts that a code role's declared env var NAMES
// (EnvPassthrough/EnvPrefix) ride a reserved fleet_env system message — names
// only, values never touched by fleet.
func TestRun_EmitsFleetEnvMessage(t *testing.T) {
	llm := &stubLLM{
		script: []ChatResult{
	...
	_, err := Run(context.Background(), Input{
		Instruction:    "code role body",
		Model:          "codellm",
		MaxTokens:      10000,
		MaxSteps:       10,
		EnvPassthrough: []string{"KRISP_TOKEN", "KRISP_BASE_URL"},
	...
	var envMsg *Message
	for i := range llm.seen {
		if llm.seen[i].Name == fleetEnvMessageName {
			envMsg = &llm.seen[i]
			break
		}
	}
	if envMsg == nil {
		t.Fatalf("expected a %q system message, none found", fleetEnvMessageName)
	}
	var got fleetEnvNames
	if uerr := json.Unmarshal([]byte(envMsg.Content), &got); uerr != nil {
		t.Fatalf("fleet_env content not JSON: %v", uerr)
	}
	if len(got.Passthrough) != 2 || got.Passthrough[0] != "KRISP_TOKEN" || got.Prefix[0] != "KRISP_" {
		t.Fatalf("unexpected fleet_env names: %+v", got)
	}
}

// TestRun_NoFleetEnvMessageWhenUnset asserts a role with no env declarations
// emits no fleet_env message (so a real LLM never sees a spurious block).
func TestRun_NoFleetEnvMessageWhenUnset(t *testing.T) {
	llm := &stubLLM{
		script: []ChatResult{{ToolCalls: []ToolCall{toolCall("1", toolFinish, map[string]any{"answer": "ok"})}}},
	}
	_, err := Run(context.Background(), Input{
		Instruction: "body", Model: "m", MaxTokens: 1000, MaxSteps: 5,
		LLM: llm, KB: newMemKB(nil),
	})
	if err != nil {
		t.Fatalf("run failed: %v", err)
	}
	for _, m := range llm.seen {
		if m.Name == fleetEnvMessageName {
			t.Fatalf("unexpected fleet_env message when no env declared")
		}
	}
}
