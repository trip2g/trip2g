...
func TestResolve(t *testing.T) {
	...
	tests := []struct {
		name     string
		input    model.CreateGoogleOAuthCredentialsInput
		mockFunc func() *envMock
		want     model.CreateGoogleOAuthCredentialsOrErrorPayload
		wantErr  bool
	}{
		{
			...
			mockFunc: func() *envMock {
				...
				mock.ValidateGoogleOAuthCredentialsFunc = func(ctx context.Context, clientID, clientSecret string) error {
					return nil
				}
				mock.EncryptDataFunc = func(plaintext []byte) ([]byte, error) {
					return []byte("encrypted-" + string(plaintext)), nil
				}
				mock.DeactivateAllGoogleOAuthCredentialsFunc = func(ctx context.Context) error {
					return nil
				}
				mock.InsertGoogleOAuthCredentialsFunc = func(ctx context.Context, arg db.InsertGoogleOAuthCredentialsParams) (db.GoogleOauthCredential, error) {
					require.True(t, arg.Active, "new credentials should be active")
					return db.GoogleOauthCredential{
						ID:        1,
						Name:      arg.Name,
			...
			want: &model.CreateGoogleOAuthCredentialsPayload{
				Credentials: &db.GoogleOauthCredential{
					ID:        1,
					Name:      "Production",
					ClientID:  "1234567890.apps.googleusercontent.com",
					Active:    true,
		...
		},
		{
			...
			mockFunc: func() *envMock {
				...
				mock.InsertGoogleOAuthCredentialsFunc = func(ctx context.Context, arg db.InsertGoogleOAuthCredentialsParams) (db.GoogleOauthCredential, error) {
					return db.GoogleOauthCredential{}, errors.New("UNIQUE constraint failed: google_oauth_credentials.client_id")
				}
				return mock
			},
			want: &model.ErrorPayload{
				Message: "Google OAuth credentials with this client ID already exist",
			},
			wantErr: false,
		},
		{
			...
			mockFunc: func() *envMock {
				...
				mock.InsertGoogleOAuthCredentialsFunc = func(ctx context.Context, arg db.InsertGoogleOAuthCredentialsParams) (db.GoogleOauthCredential, error) {
					return db.GoogleOauthCredential{}, errors.New("database error")
				}
				return mock
			},
			want:    nil,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			env := tt.mockFunc()
			got, err := creategoogleoauthcredentials.Resolve(context.Background(), env, tt.input)
			...
			case *model.CreateGoogleOAuthCredentialsPayload:
				gotPayload, ok := got.(*model.CreateGoogleOAuthCredentialsPayload)
				require.True(t, ok, "expected CreateGoogleOAuthCredentialsPayload")
