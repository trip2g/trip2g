// Code generated by qtc from "views.qtpl". DO NOT EDIT.
// See https://github.com/valyala/quicktemplate for details.

//line internal/case/render404/views.qtpl:1
package render404

//line internal/case/render404/views.qtpl:1
import (
	qtio422016 "io"

	qt422016 "github.com/valyala/quicktemplate"
)

//line internal/case/render404/views.qtpl:1
var (
	_ = qtio422016.Copy
	_ = qt422016.AcquireByteBuffer
)

//line internal/case/render404/views.qtpl:1
func StreamContent(qw422016 *qt422016.Writer, params *Params) {
//line internal/case/render404/views.qtpl:1
	qw422016.N().S(`
<div mol_view_root="$trip2g_user_space"></div>
<div>
  Страница не найдена.
  `)
//line internal/case/render404/views.qtpl:5
	if params.UserToken == nil {
//line internal/case/render404/views.qtpl:5
		qw422016.N().S(`
  Ее не существует или она доступна только после авторизации.
  `)
//line internal/case/render404/views.qtpl:7
	}
//line internal/case/render404/views.qtpl:7
	qw422016.N().S(`
</div>
<a href="/">Home</a>
`)
//line internal/case/render404/views.qtpl:10
}

//line internal/case/render404/views.qtpl:10
func WriteContent(qq422016 qtio422016.Writer, params *Params) {
//line internal/case/render404/views.qtpl:10
	qw422016 := qt422016.AcquireWriter(qq422016)
//line internal/case/render404/views.qtpl:10
	StreamContent(qw422016, params)
//line internal/case/render404/views.qtpl:10
	qt422016.ReleaseWriter(qw422016)
//line internal/case/render404/views.qtpl:10
}

//line internal/case/render404/views.qtpl:10
func Content(params *Params) string {
//line internal/case/render404/views.qtpl:10
	qb422016 := qt422016.AcquireByteBuffer()
//line internal/case/render404/views.qtpl:10
	WriteContent(qb422016, params)
//line internal/case/render404/views.qtpl:10
	qs422016 := string(qb422016.B)
//line internal/case/render404/views.qtpl:10
	qt422016.ReleaseByteBuffer(qb422016)
//line internal/case/render404/views.qtpl:10
	return qs422016
//line internal/case/render404/views.qtpl:10
}
