...
func staticTools(ctx context.Context, env Env) []Tool { //nolint:funlen // flat declarative list of built-in tool schemas
	...
	tools := []Tool{
		...
		{
			Name:        "expand",
			Description: "Walk a note's table of contents level by level (progressive disclosure). Canonical call: expand(path=<result.note_path>, toc_path=[...]) — copy path verbatim from a search result's note_path field. Returns the direct children of a TOC node: omit toc_path (or pass []) for the top-level sections, or pass a toc_path to list that section's subsections. Each child has title, level, path, and has_children. A section without subsections is returned in full (the same text note_html gives for that toc_path, plus section_html in the payload), so descending with expand ends in the read itself — no need to load the whole note or its full flat TOC.",
			InputSchema: &InputSchema{
				Type: "object",
				Properties: map[string]Property{
					"path": {
		...
		},
		{
			...
			Description: "Walk a remote note's table of contents level by level inside a connected knowledge base (progressive disclosure), same as expand. Canonical call: federated_expand(kb_id=..., path=<result.note_path>, toc_path=[...]). Omit toc_path for the top level, or pass a toc_path to list that node's subsections. A section without subsections is returned in full, as federated_note_html would return it.",
			InputSchema: &InputSchema{
				Type: "object",
				Properties: map[string]Property{
					"kb_id": {
