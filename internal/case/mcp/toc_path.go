...
type TOCItem struct {
	...
	// ID is the heading's anchor in the rendered page, generated by the
	// markdown pipeline and already used by the TOC widget.
...
// snippetTocPath is tocPathForSnippet in the note's frame: the breadcrumb is
// walked out of the rendered HTML, where the title H1 still wraps the whole
// document, but expand drops that H1 — so a toc_path from search and one from
// expand have to be trimmed the same way or they describe different shapes.
func snippetTocPath(note *model.NoteView, snippet, chunkContent string) []string {
	if note == nil {
		return nil
	}
	path := tocPathForSnippet(string(note.HTML), snippet, chunkContent)
	if note.HasH1 && len(note.Headings) > 0 && len(path) > 0 && path[0] == note.Headings[0].Text {
		path = path[1:]
	}
	if len(path) == 0 {
		return nil
...
// sectionAnchorURL returns a link straight to the matched section, using the
// heading id the page already renders. Empty when the path names no heading —
// the note's own URL is the answer then.
func sectionAnchorURL(note *model.NoteView, tocPath []string, noteURL func(*model.NoteView) string) string {
	if note == nil || len(tocPath) == 0 || noteURL == nil {
		return ""
	}
	for _, item := range buildNoteTOC(navigableHeadings(note)) {
		if item.ID == "" || len(item.Path) != len(tocPath) || !tocPathHasPrefix(item.Path, tocPath) {
			continue
		}
		return noteURL(note) + "#" + item.ID
	}
	return ""
}
