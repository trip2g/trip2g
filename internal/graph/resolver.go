...
type Env interface {
	...
	// Background Queues
	GetBackgroundQueue(ctx context.Context, name string) (*model.BackgroundQueue, error)
	ListBackgroundQueues(ctx context.Context) []model.BackgroundQueue
	StopBackgroundQueue(ctx context.Context, name string) error
	StartBackgroundQueue(ctx context.Context, name string) error
	ClearBackgroundQueue(ctx context.Context, name string) (int64, error)

	// goqite SQL queries - these are generated by sqlc from queries.read.sql
	GetGoqiteQueueStats(ctx context.Context, queue string) (db.GetGoqiteQueueStatsRow, error)
	ListGoqiteJobsByQueue(ctx context.Context, arg db.ListGoqiteJobsByQueueParams) ([]db.ListGoqiteJobsByQueueRow, error)

	// Telegram custom emojies
	ListAllTelegramCustomEmojies(ctx context.Context) ([]db.TelegramCustomEmojy, error)
}
